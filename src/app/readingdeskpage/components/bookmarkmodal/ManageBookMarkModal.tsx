...
export default function ManageBookMarkModal({ onClose }: BookMarkModalProps) {
		...
		<Modal onClose={onClose}>
			<CustomColumn $width="100%" $height="auto" $alignitems="flex-start" $justifycontent="center" $gap="2rem">
				...
				<CustomColumn $width="100%" $gap="1rem">
					{bookmarks.map((bookmark, index) => (
						...
						>
							...
							<NormalInputField defaultValue={bookmark.text} />
							<CustomButton
								$width="auto"
								$height="auto"
				...
				<CustomRow $width="100%" $height="auto" $alignitems="center" $justifycontent="flex-end">
					<ArrowButton
						imagePath="/modal_back_button_small.svg"
						width="w-[6rem]"
						height="h-[2rem]"
						text="저장"
						onClick={onClose}
