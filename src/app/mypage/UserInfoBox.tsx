    ...
    return (
        <CustomRow $justifycontent="center" $gap="8rem" $width="90%">
            ...
            <CustomColumn $width='70%' $alignitems="flex-start" $justifycontent="center">
                ...
                <CustomButton $width='100%' $height='auto' $backgroundColor="#7A6B52" $color="white" $padding='0.2rem 0.5rem'
                    onClick={handleOpenLogoutModal}>
                    <span className="font-bold">로그아웃</span>
                </CustomButton>
                <CustomButton $width='100%' $height='auto' $color="#7A6B52" $border="1px solid #7A6B52" onClick={handleOpenUnlinkModal} $padding='0.2rem 0.5rem'
                    $backgroundColor="transparent">
                    <span className="font-bold">회원탈퇴</span>
                </CustomButton>
                {isUnlinkModalOpen && (
                    <UnlinkModal onClose={handleCloseUnlinkModal} />
                )}
